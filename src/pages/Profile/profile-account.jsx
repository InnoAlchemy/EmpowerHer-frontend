...
const ProfileAccount = () => {
  const { user } = useUser();
    const { username,email, membership, profile_picture,country,phone_number,job_title } = user;
  // State for dropdowns
  ...
  <h2 className={`text-2xl text-[#7A89C2] font-semibold mt-10 ${membership === null ? 'pointer-events-none opacity-50' : ''}`}>
    Organization Information (Available with subscription)
  </h2>

  <div className={`flex flex-col space-y-4 ${membership === null ? 'pointer-events-none opacity-50' : ''}`}>
          ...
          {productFile && (
            <div className="bg-[#EADFE2] rounded-[20px] p-1 flex items-center space-x-2">
              <span className="text-[#7A89C2]">
                {productFile.name} <span className="ml-2 text-[#00000099]" >({productFile.size} kb)</span>
              </span>
              <button
